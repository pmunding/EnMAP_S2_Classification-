## in diesem Script wird ein RF-Modell zur Klassifikation der EnMAP-Daten erstellt
...
model <- train(trainDat[,predictors],
               ...
               method="rf",
               importance=TRUE,
               ntree=500,              # ntree = 500 als optimal ausgewählt
               tuneGrid = data.frame(mtry = 15),,
               trControl=ctrl)
model

saveRDS(model,file="RFmodelE.RDS") # Modell speichern
...
load("RFmodelE.RData")
save(model, file = "RFmodelE.RData")
model <- readRDS("RFmodelE.RDS")
...
# Importance-Matrix extrahieren
imp <- varImp(model)$importance
imp$Variable <- rownames(imp)

imp_long <- imp %>%
  pivot_longer(cols = -Variable, names_to = "Klasse", values_to = "Wichtigkeit")

# Für jede Klasse: Top 10 Variablen mit höchster Wichtigkeit auswählen
top_per_class <- imp_long %>%
  group_by(Klasse) %>%
  slice_max(order_by = Wichtigkeit, n = 10) %>%
  arrange(Klasse, desc(Wichtigkeit))
